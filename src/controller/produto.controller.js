...
const findAllProductscontroller = async (req, res) => {
  try {
    res.send(
      await produtoService.findAllProductsService(
        req.query.limit,
        req.query.offset
      )
    );
...
const addCategoriaProdutoController = async (req, res) => {
  try {
    res
      .status(200)
      .send(
        await produtoService.addCategoriaProdutoService(req.params.id, req.body)
      );
  } catch (err) {
    console.log(`erro: ${err.message}`);
    return res
...
const removeCategoriaProdutoController = async (req, res) => {
  try {
    res
      .status(200)
      .send(
        await produtoService.removeCategoriaProdutoService(
          req.params.id,
          req.body
        )
      );
